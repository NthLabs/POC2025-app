...
# My Variables
webTitle = "RAG Demo"               # Title on Browser
myDocs = "./data/RAGdemo"           # Location for PDFs
vsPath = "./catalog/RAGdemo"        # Location for VectorStore
logFile = "./logs/RAGprompt.log"    # Location for Prompt Logs
perfLog = "./logs/RAGperf.log"      # Location for Perf Log
logo = "images/NthLabs.png"         # 
msgHistory = "messagesRAG"          # This should be unique for each streamlit page
vsName = "RAGdemo"                  # This should be unique for each Chroma instance
...
def get_no_rag_response(userInput):
    prompt = ChatPromptTemplate.from_messages([
    ...
    if IncludeSource:
        response += '\n\n'
        response += 'No Sources... Not using Knowledgebase'
...
def generate_response(userInput):
    ...
    with st.chat_message('assistant'):
        if UseKnowledge:
            ...
            response = get_no_rag_response(userInput)
        st.write(response)
...
#tab1, tab2, tab3 = st.tabs(["Chat Interface", "Knowledgebase", "PDFs"])
tab1, tab2, = st.tabs(["Chat Interface", "Knowledgebase"])

# Sidebar
UseKnowledge = st.sidebar.toggle("Use Knowledgebase")
IncludeSource = st.sidebar.toggle("Cite Sources")
...
with tab2:
    st.subheader("List of Files in Knowledgebase:")
